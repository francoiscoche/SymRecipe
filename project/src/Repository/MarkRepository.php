...
class MarkRepository extends ServiceEntityRepository
{
    ...
    /**
     * return notation by recipe
     *
     * @param integer|null $idRecipe
     * @return void
     */
    public function getAverageNotationByRecipe(?int $idRecipe) 
    {
        $val = 0;

        $queryBuilder = $this->createQueryBuilder('m')
            ->where('m.recette = :idrecipe')
            ->setParameter('idrecipe', $idRecipe)
            ->getQuery()
            ->getResult();

        foreach($queryBuilder as $arr) {
            $val += $arr->getMark();
        }

        return ($val / count($queryBuilder));
    }
